...
Pressing `delete` on the selected task SHALL drop it (transition to dropped) when its status is pending or done, after confirmation. When the selected task is already dropped, `delete` SHALL be a no-op and SHALL NOT be advertised in the help bar.
...
#### Scenario: Drop a done task
- **WHEN** the selected task is done and the user presses `delete` and confirms
- **THEN** the task is dropped via DropTask and its status becomes dropped
...
The move bindings (shift+up / shift+down) SHALL be available only when the selected task is pending. For a done or dropped selection the move bindings SHALL NOT be advertised in the help bar, and pressing them SHALL have no effect.
...
#### Scenario: Move bindings available for a pending task
- **WHEN** the selected task is pending
- **THEN** the help bar shows the move-up/move-down bindings
