...
The system SHALL provide a `screen.Overlay(parent, child)` function that wraps a child Screen with a parent reference. The overlay SHALL handle esc by sending `screen.Dismiss()` when the inner screen is not capturing input.
...
#### Scenario: Overlay KeyMap includes esc
- **WHEN** the overlay's KeyMap is rendered
- **THEN** it SHALL include the esc binding from the wrapper
- **AND** it SHALL include all bindings from the inner screen's KeyMap
